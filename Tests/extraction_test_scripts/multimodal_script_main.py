...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("multimodal_extraction_test.log"),
...
def generate_extraction(model, tokenizer, prompt, image, use_loop_prevention=True):
    ...
    try:
        ...
        if use_loop_prevention:
            # Use the loop prevention mechanism
...
def run_tests(test_dir: str, output_dir: str):
    ...
    for i, product_file in enumerate(product_files):
        try:
            ...
            # image = get_image_for_datasheet_page(product_data['datasheet_url'], product_data['page'])
            ...
            extracted_graph, raw_responses, loops_detected = extract_graph_from_image(model, tokenizer, image, product_data)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run multimodal parameter extraction tests')
